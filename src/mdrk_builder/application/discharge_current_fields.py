...
from mdrk_builder.application.discharge_extractors import extract_discharge_clinical_sections, extract_discharge_final_fields
...
from mdrk_builder.domain import ReviewIssue, ReviewSeverity, SpecialistRole
...
def select_current_fields(documents, admission, discharge, mis_path=None):
    ...
    for key, rows in candidates.items():
        ...
        latest = [(value, path) for stamp, value, path in rows if stamp.date() == latest_day]
